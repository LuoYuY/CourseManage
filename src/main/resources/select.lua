...
-- 商品库存key
local product_stock_key = 'classid:' .. product_id .. ':stock'
-- 商品秒杀结束标识的key
local end_product_key = 'classid:' .. product_id .. ':end'

-- 存储秒杀成功的用户id的集合的key
local bought_users_key = 'classid:' .. product_id .. ':uids'

--判断该商品是否秒杀结束
local is_end = redis.call('get',end_product_key)

if tonumber(is_end) ==1 then
    return -1
end
